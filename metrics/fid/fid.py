...
from .inceptionv4 import get_inceptionv4_model
...
def get_activations(images, model, batch_size=64, dims=2048,
    ...
    for i in range(n_batches):
        ...
        if cuda:
            batch = batch.to(cuda)#torch.from_numpy(images[start:end]).type(torch.FloatTensor)
...
def calculate_fid_given_paths(paths, batch_size, cuda, dims):
    ...
    inception_model = get_inceptionv4_model() #InceptionV3([block_idx]).type(dtype)
    if cuda:
